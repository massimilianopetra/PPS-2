...
uint16_t mos6502::Print()
{
	...
	if (status & INTERRUPT) printf("I");
	if (status & ZERO) printf("Z");
	if (status & CARRY) printf("C");
	
	printf("\n");
	return pc;	
}

uint16_t mos6502::Dump(uint8_t* _A, uint8_t* _X,uint8_t* _Y,uint8_t* _SP, uint8_t* _P)
{
	*_A=A;
	*_X=X;
	*_Y=Y;
